...
function setBulbColor(installedApp, token) {
  const area = installedApp.config.area[0].stringConfig.value;
  const deviceId = installedApp.config.colorLight[0].deviceConfig.deviceId;
  const forecastInterval = installedApp.config.forecastInterval[0].stringConfig.value;
  console.log(`FORECAST INTERVAL SELECTED IS: ${forecastInterval}`);

  const chunks = stConfig.getForecastChunks(forecastInterval);

  weather.getForecast(forecastInterval, area)
  .then(function(forecast) {
    const color = weather.getColorForForecast(forecast, area);
